...
static void neo2_isr_handler(void* arg){
}
...
esp_err_t ws2812_control_init(void)
{
  ...
  //rmt_isr_register(neo2_isr_handler, NULL, 0, NULL);
  ESP_LOGI(TAG, "Allocated TX RMT channel %d",config.channel);
  rmt_set_tx_intr_en(LED_RMT_TX_CHANNEL,true);
